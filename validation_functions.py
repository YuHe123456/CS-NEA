...
# Validate a coefficient - called by validate_preset
...
# Validate a full text file, containing multiple preset strings - calls validate_preset on each preset
...
def validate_preset_file(preset_file: Path, error_file: Path) -> bool:
    ...
    for i, preset in enumerate(preset_list): # Validate each preset
    ...
    print(f"File name: {preset_file.name}\nValid Files: {valid_preset_count}\nInvalid Files: {invalid_preset_count}") 
...
if __name__ == "__main__":
    ...
    print(validate_preset_file(base / "presets" / "test.txt", base / "errors" / "errors.txt"))